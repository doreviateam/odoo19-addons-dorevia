...
| **Recalcul** | **Interdit** : `dorevia_cash_flow` ne doit **jamais** appeler automatiquement `action_recompute_projection()`. Si `weekly_line_ids` est vide ou manifestement obsolète : **message** invitant l’utilisateur à ouvrir le document dans **Cash Guards** (liste atelier : **Comptabilité > Projection de trésorerie > Cash Guards**) et à y déclencher le recalcul. |
...
| `weekly_line_ids` vide ou données manifestement absentes | **Aucun** appel à `guard.action_recompute_projection()` depuis `dorevia_cash_flow`. Afficher un **message explicite** invitant l’utilisateur à ouvrir le document dans **Cash Guards** (**Comptabilité > Projection de trésorerie > Cash Guards**) et à y lancer le recalcul / actualisation. Ne pas ouvrir de vue Graph vide sans explication. |
...
2. **Cash Flow sous Comptabilité > Projection de trésorerie** : entrée **Cash Flow** dans `views/cash_guard_bridge_menus.xml` (parent = **`dorevia_cash_guard.menu_dorevia_cash_guard_root`**) — même résolution de référence et même client action que la trajectoire ; paramètre `cockpit` sur l’action client pour masquer les contrôles d’audit sur le graphique et afficher les raccourcis atelier (sans second moteur graphique). Sous-menu **Cash Guards** sur la même racine pour la liste des projections. Menus **Analyse** renommés **Trajectoire (Analyse)** / **Trajectoire — choix projection (Analyse)** (raccourcis secondaires, séquence basse dans la liste).
