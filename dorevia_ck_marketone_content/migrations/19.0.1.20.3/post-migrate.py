# -*- coding: utf-8 -*-
"""Section 3 — reconstruction home obligatoire (étiquettes produit sur cards SSR)."""
...
def migrate(cr, version):
    ...
    if not bootstrap_home_featured_products(env):
        raise RuntimeError('CK featured migration 20.3: bootstrap_home_featured_products a échoué')
    ...
    if _featured_arch_missing_product_labels(env, arch, variants):
        raise RuntimeError(
            'CK featured migration 20.3: étiquettes produit toujours absentes de la home'
        )
    cr.commit()
