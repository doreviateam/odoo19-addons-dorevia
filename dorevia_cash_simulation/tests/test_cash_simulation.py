...
class TestCashSimulation(TransactionCase):
    """Tests V1 for dorevia_cash_simulation — sale order simulation in Cash Guard."""
    ...
    def _create_quote(self, amount=1000.0, simulation=False, due_date=None, state="draft"):
    ...
    def test_activation_without_date_raises(self):
        order = self._create_quote(simulation=False)
    ...
    def test_date_change_to_past_on_active_simulation_raises(self):
        order = self._create_quote(simulation=True, due_date=self._future())
        with self.assertRaises(ValidationError):
    ...
    def test_unrelated_write_on_stale_simulation_ok(self):
        ...
        order = self._create_quote(simulation=True, due_date=self._future(1))
    ...
    def test_eligible_sent_simulation(self):
        order = self._create_quote(simulation=True, due_date=self._future(), state="sent")
        self.assertTrue(order.cash_simulation_eligible)
    ...
    def test_search_excludes_other_company(self):
        ...
        orders = guard._search_eligible_simulation_orders()
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 4. Projection Cash Guard
    # ──────────────────────────────────────────────────────────────────────

    def test_simulation_off_no_impact(self):
        self._create_quote(simulation=True, due_date=self._future(), amount=5000)
    ...
    def test_simulation_bucket_correct_week(self):
        due = self._future(20)
        self._create_quote(simulation=True, due_date=due, amount=3000)
        guard = self._create_guard(include_simulation=True)
        self._recompute_with_zero_balance(guard)
        meta = guard._split_exercise_periods()
        sit = guard.situation_date
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 5. Recompute au toggle
    # ──────────────────────────────────────────────────────────────────────

    def test_toggle_triggers_recompute(self):
        self._create_quote(simulation=True, due_date=self._future(), amount=2000)
        guard = self._create_guard(include_simulation=False)
        self._recompute_with_zero_balance(guard)
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        guard.write({"include_simulation": True})
    ...
    def test_simulation_order_count(self):
        self._create_quote(simulation=True, due_date=self._future())
        self._create_quote(simulation=True, due_date=self._future())
        self._create_quote(simulation=False)
        guard = self._create_guard(include_simulation=True)
