# Recette manuelle — dorevia_glc_analytics · Palier 0
...
docker compose run --rm odoo odoo -c /etc/odoo/odoo.conf \
  -d glc-rgl-test-import -u dorevia_glc_analytics --stop-after-init --no-http
...
| # | Contrôle | ☐ |
|---|---|---|
| A1 | Module `dorevia_glc_analytics` installé, état **Installé** (Apps) | ☐ |
...
|---|---|---|---|---|
| P0.1 | Apps → rechercher « GLC Analytics » ou `dorevia_glc_analytics` | Module **Dorevia GLC Analytics** visible, état **Installé** | ☐ | |
...
**Action :** relancer `-u dorevia_glc_analytics` sur la base.
...
| Redémarrage Odoo | OK |
| Tests automatisés (`/dorevia_glc_analytics`) | **7 tests, 0 échec, 0 erreur** |
