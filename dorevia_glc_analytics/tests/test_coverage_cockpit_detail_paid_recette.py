...
class TestGlcCoverageCockpitDetailPaidRecette(TestGlcCoverageCockpitTreasury):
    ...
    def test_rt_pay_10_budget_variance_unchanged(self):
        """RT-PAY-10 — budgets inchangés ; écarts recalculés sur montants payés."""
        year = self._recette_year()
        period = "%s-06-01" % year
        invoice_date = self._recette_date(year).isoformat()
        self._create_validated_budget(
            ...
            [
                ...
                },
                {
                    "period_date": period,
                    "line_type": "expense",
                    "analytic_account_id": self.prestations.id,
                    "amount": 1500.0,
                },
            ],
        )
        ...
        self.assertAlmostEqual(bar.revenue_budget, 5000.0)
        self.assertAlmostEqual(structure.expense_budget, 2000.0)
        # Budget identique ; écart payé ≠ écart engagé
        variance_engaged = bar.revenue_realized - bar.revenue_budget
        variance_paid = bar.revenue_realized_paid - bar.revenue_budget
        self.assertNotAlmostEqual(variance_engaged, variance_paid)
        self.assertAlmostEqual(
            variance_paid, self.AMT_REV_PAID - bar.revenue_budget
        )
