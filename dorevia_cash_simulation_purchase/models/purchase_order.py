...
class PurchaseOrder(models.Model):
    ...
    cash_simulation_ok = fields.Boolean(
        ...
        tracking=True,
        help=(
            "Permet d'utiliser cette demande de prix / commande achat comme "
            "hypothèse de décaissement futur dans Cash Guard lorsque le mode "
    ...
    cash_simulation_due_date = fields.Date(
        ...
        tracking=True,
        help=(
            "Date prévisionnelle de décaissement utilisée uniquement pour la "
            "simulation de trésorerie. Indépendante de la date de commande."
        ),
    )
        ...
        for order in self:
            ...
            if not order.cash_simulation_due_date:
                raise ValidationError(
                    _(
                        "La commande achat « %(name)s » ne peut pas être incluse "
                        "dans la simulation sans date d'échéance de simulation.",
            ...
            if order.cash_simulation_due_date <= today:
                raise ValidationError(
                    _(
                        "La commande achat « %(name)s » : la date d'échéance de "
                        "simulation doit être postérieure à aujourd'hui.",
                        name=order.display_name,
                    )
                )
