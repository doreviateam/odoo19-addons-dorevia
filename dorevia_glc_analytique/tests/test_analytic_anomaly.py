...
class TestGlcAnalyticAnomaly(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        manager_group = cls.env.ref("dorevia_glc_analytique.group_glc_manager")
        cls.env.ref("base.user_admin").group_ids += manager_group
        cls.env = cls.env(user=cls.env.ref("base.user_admin"))

        plan_ids = [
            cls.env.ref("dorevia_glc_analytique.analytic_plan_glc_activites").id,
            cls.env.ref("dorevia_glc_analytique.analytic_plan_glc_financements").id,
        ]
        cls.env["account.analytic.account"].sudo().search(
            [("plan_id", "in", plan_ids)]
        ).write({"company_id": cls.env.company.id})

        cls.bar = cls.env.ref("dorevia_glc_analytique.analytic_account_glc_bar")
        cls.ressources = cls.env.ref(
            "dorevia_glc_analytique.analytic_account_glc_ressources_propres"
        )
        cls.structure = cls.env.ref("dorevia_glc_analytique.analytic_account_glc_structure")
        cls.adhesions = cls.env.ref("dorevia_glc_analytique.analytic_account_glc_adhesions")
    ...
    def test_menu_anomaly_wizard_exists(self):
        ...
        menu = self.env.ref("dorevia_glc_analytique.menu_glc_analytic_anomalies")
        action = self.env.ref("dorevia_glc_analytique.action_glc_analytic_anomaly_wizard")
        self.assertEqual(menu.action.id, action.id)
        self.assertIn(
            self.env.ref("dorevia_glc_analytique.group_glc_manager"),
            menu.group_ids,
        )
    ...
    def test_a5_legacy_account_after_cutover(self):
        ...
        icp.set_param("dorevia_glc_analytique.cutover_date", "2026-01-01")

        legacy_plan = self.env.ref("dorevia_glc_analytique.analytic_plan_glc_activites")
