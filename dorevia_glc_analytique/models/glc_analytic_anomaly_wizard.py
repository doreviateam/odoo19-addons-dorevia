...
class GlcAnalyticAnomalyWizard(models.TransientModel):
    ...
    def _update_control_status(self):
        ...
        cutover = icp.get_param("dorevia_glc_analytique.cutover_date")
        ...
        vals = {
            ...
            "a5_info_message": (
                ...
                if cutover
                    ...
                    "Contrôle A5 inactif : paramètre dorevia_glc_analytique.cutover_date absent."
                )
            ),
        }
        self.write(vals)
    ...
    def _get_plan(self, xmlid):
        return self.env.ref(f"dorevia_glc_analytique.{xmlid}")
    ...
    def _check_a5(self, line, distribution, common):
        icp = self.env["ir.config_parameter"].sudo()
        cutover_raw = icp.get_param("dorevia_glc_analytique.cutover_date")
        if not cutover_raw:
            return []
    ...
    def _compute_structure_weight(self, move_lines):
        ...
        icp = self.env["ir.config_parameter"].sudo()
        threshold = float(
            icp.get_param("dorevia_glc_analytique.structure_weight_alert_pct", "30")
        )
