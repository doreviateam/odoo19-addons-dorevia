# Scénario manuel — Cash Guard V1.2 (solde projeté / factures ouvertes)

**Ticket** : `CG-V1.2-01-PROJECTED-BALANCE-FROM-OPEN-INVOICES`  
**Objectif** : valider la colonne **Solde projeté** dans le suivi de trésorerie et son calcul depuis les factures **postées** avec résiduel, sans budget ni simulations.
...
2. Noter le **Solde** (constaté) sur la ligne « Situation » et le **Solde projeté** sur cette même ligne (sans facture ouverte : ils doivent être identiques au solde constaté sur cette maille).
...
5. Vérifier dans **Suivi de trésorerie** :
   - une ligne **Prévisionnel** correspondant à la semaine de l’échéance affiche un **Solde projeté** supérieur au **Solde** de cette ligne d’un montant égal au **résiduel** de la facture (en devise société) ;
   - le **Statut** des lignes utilise bien le **Solde projeté** (pas uniquement le solde constaté si la facture suffit à changer le niveau de risque).

6. **Créer une facture fournisseur validée**, non payée, échéance sur une autre semaine.

7. **Actualiser** ; constater une **baisse** du **Solde projeté** sur la période contenant cette échéance (impact opposé au client).
...
11. **Actualiser** ; vérifier **aucun impact** sur le **Solde projeté** (les brouillons sont exclus).

12. Créer une **facture client validée**, non payée, avec une **date d’échéance antérieure à la date de situation**.

13. **Actualiser** ; vérifier qu’elle impacte le **Solde projeté** dès la ligne **Situation** ou la période contenant la date de situation.

14. Créer une **facture brouillon / à valider** avec une date d’échéance **antérieure** à la date de situation.

15. **Actualiser** ; vérifier qu’elle **n’impacte pas** le **Solde projeté**.

---

## Résultat attendu

- Le solde projeté reflète **uniquement** les pièces **postées** avec résiduel non nul, agrégées depuis `account.move`.
