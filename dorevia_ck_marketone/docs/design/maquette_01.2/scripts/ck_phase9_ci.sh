#!/usr/bin/env bash
# Gate portable Phase 9 — newsletter M9 · sans Playwright.
#
#   1. upgrade dorevia_ck_theme
#   2. tests Odoo --test-tags=dorevia_ck_theme_phase9
#   3. smoke curl contact · pro · subscribe markers · non-régression Phases 1–8
...
TEST_TAG="dorevia_ck_theme_phase9"
...
log "Gate portable Phase 9 · base=${DB} · modules=${MODULES}"
...
curl_smoke "/contactus" "ck-contact-page" "contactus_form" "ck-dual-engage--compact" "ck-newsletter-subscribe" "Recevez les nouveautés créoles" "Désinscription possible" "<html" "web.assets_frontend"
curl_smoke_absent "/contactus" "s_newsletter_subscribe_popup"
curl_smoke "/professionnels" "ck-pro-page" "ck-pro-form" "ck-dual-engage--compact" "ck-newsletter-subscribe" "crm.lead"
curl_smoke "${RECIPES_PATH}" "ck-recipes-page"
curl_smoke "${PRODUCER_PATH}" "ck-producer-page"
...
grep -qF 'ck-dual-engage' "$home_tmp" || fail "Smoke / : dual home Phase 2 absent"
rm -f "$home_tmp"
printf "  ✓ / → 200 · vedettes SSR · dual home Phase 2\n"

log "Gate portable Phase 9 : OK"
log "Recette UX complémentaire (optionnelle) :"
printf '  node %s/ck_phase9_desktop1280.mjs\n' "$ROOT"
printf '  node %s/ck_phase9_mobile390.mjs\n' "$ROOT"
