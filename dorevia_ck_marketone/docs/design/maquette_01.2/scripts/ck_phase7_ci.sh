#!/usr/bin/env bash
# Gate portable Phase 7 — fiche producteur pilote M1 · sans Playwright.
#
#   1. upgrade dorevia_ck_theme
#   2. tests Odoo --test-tags=dorevia_ck_theme_phase7
#   3. smoke curl producteur · non-régression Phases 1–6
...
TEST_HTTP_PORT="${CK_CI_TEST_HTTP_PORT:-8076}"
MODULES="dorevia_ck_theme,dorevia_ck_marketone_content"
TEST_TAG="dorevia_ck_theme_phase7"
...
log "Gate portable Phase 7 · base=${DB} · modules=${MODULES}"
...
curl_smoke "${PRODUCER_PATH}" "ck-producer-page" "Atelier Les Hauts Goyaviers" "ck-producer-products" 'href="/shop"' "<html" "web.assets_frontend"
curl_smoke_absent "${PRODUCER_PATH}" "/recettes" "fiche-produit.html"
curl_smoke "/contactus" "ck-contact-page" "<html" "web.assets_frontend"
curl_smoke "/a-propos" "ck-about-page" "<html" "web.assets_frontend"
curl_smoke "/professionnels" "ck-pro-page" "ck-pro-form" "<html" "web.assets_frontend"
curl_smoke "${PRODUCT_PATH}" "ck-product-page" "ck-product-pro-signal"
curl_smoke_absent "${PRODUCT_PATH}" "atelier-hauts-goyaviers"
...
log "Gate portable Phase 7 : OK"
log "Recette UX complémentaire (optionnelle) :"
printf '  node %s/ck_phase7_desktop1280.mjs\n' "$ROOT"
printf '  node %s/ck_phase7_mobile390.mjs\n' "$ROOT"
