#!/usr/bin/env bash
# Gate portable Phase 4 — fiche produit CK · sans Playwright.
#
#   1. upgrade dorevia_ck_theme
#   2. tests Odoo --test-tags=dorevia_ck_theme_phase4
#   3. smoke curl fiche · /shop/cart · /shop · /professionnels · /
#
# Usage :
#   ./ck_phase4_ci.sh
#   CK_CI_DB=ma_base CK_CI_BASE_URL=http://localhost:8069 ./ck_phase4_ci.sh
#
# Variables :
...
#   CK_CI_TEST_HTTP_PORT  Port HTTP tests Odoo (défaut : 8073)
#   CK_CI_SKIP_RESTART    Si "1", ne pas redémarrer le conteneur après upgrade
#
# Playwright (ck_phase4_desktop1280.mjs · mobile390) : recette UX complémentaire, hors gate.
...
TEST_HTTP_PORT="${CK_CI_TEST_HTTP_PORT:-8073}"
MODULES="dorevia_ck_theme,dorevia_ck_marketone_content"
TEST_TAG="dorevia_ck_theme_phase4"
...
discover_product_path() {
  ...
  rm -f "$shop_tmp"
  [[ -n "$path" ]] || fail "Aucun produit publié sur /shop — définir CK_CI_PRODUCT_PATH ou bootstrapper le catalogue"
  printf '%s' "$path"
}
...
log "Gate portable Phase 4 · base=${DB} · modules=${MODULES}"
...
curl_smoke "${PRODUCT_PATH}" "ck-product-page" "add_to_cart" "ck-product-pro-signal"
curl_smoke "/shop/cart" "o_website_sale_checkout_container"
curl_smoke "/shop" "s_ck_shop_intro"
curl_smoke "/professionnels" "professionnels"
...
grep -qF 'ck-product-pro-signal' "$home_tmp" && fail "Smoke / : signal pro produit sur home"
rm -f "$home_tmp"
printf "  ✓ / → 200 · vedettes SSR · pas de fiche Phase 4\n"

log "Gate portable Phase 4 : OK"
log "Recette UX complémentaire (optionnelle) :"
printf '  CK_CI_PRODUCT_PATH=%s node %s/ck_phase4_desktop1280.mjs\n' "$PRODUCT_PATH" "$ROOT"
printf '  CK_CI_PRODUCT_PATH=%s node %s/ck_phase4_mobile390.mjs\n' "$PRODUCT_PATH" "$ROOT"
