...
2. **Substituer visuellement** le libellé « Boutique » par une icône maison FA4 (`fa-home`, `\f015`), alignée Breadcrumb-U1, tout en conservant le texte dans le DOM pour l'accessibilité.
...
| `website_header.scss:302-314` | Icône Boutique desktop via `[href="/shop"]` + `::before` FA4 `fa-home` |
| `__manifest__.py` | `19.0.1.114.0` |
...
| `\f015` `fa-home` FontAwesome 4 | Aligné Breadcrumb-U1 — racine catalogue `/shop` |

---

## Recette

Contrôles sur `19.0.1.114.0` (centrage + icône `fa-home`).

| Contrôle | Résultat |
| --- | --- |
| Version DB `dorevia_ck_theme` | `19.0.1.114.0` |
| CSS compilé — icône | `::before { content: '\f015'; font-family: FontAwesome }` |
