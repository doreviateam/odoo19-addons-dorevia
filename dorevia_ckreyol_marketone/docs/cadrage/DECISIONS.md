...
| **Sidebar cible `/shop`** | Ordre MOA (2026-05-19, `19.0.12.1.0`) : **Collections** · **Catégories** · **Origines** · **Fourchette de prix** — ordre fixe QWeb + `flex-order` sur le rail desktop ; pas de séquence BO par rubrique dans ce correctif. La rubrique Collections **n’remplace pas** catégories ni origines. Facette `marketone_collection` — philosophie C4. **Évolution possible (non livrée)** : modèle ou champs BO `sequence` par rubrique sidebar pour réordonner sans toucher aux templates. |
