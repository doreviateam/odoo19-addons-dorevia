...
- [x] **Recette MOA (module 19.0.1.6.1)** — suite `dorevia_ckr_collections` **exécutée réellement** sur `ckr_collections_recette`, **23/23 verts** (9 Model + 14 HTTP, 13,92 s, 2 016 requêtes). Preuves : `docs/mvp_01/evidences/` (`run_rc_collections_v2_summary.log` + `README.md`). PV §4 renseigné colonne *Résultat observé* ; §7 anomalies A1/A2/A3/A4 **résolues** ; §8 **Conforme** ; §9 visas MOA / Dev posés. **Checklist §13 clôturée.**
