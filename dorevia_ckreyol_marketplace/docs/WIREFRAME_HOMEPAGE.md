...
**Libellés front et ordre d’affichage** (section **Explorer**) :

1. **Promotions** — avantage commercial ;
2. **Collections** — sélection éditoriale ;
3. **Kits** *(libellé visiteur ; grille back-office / source de vérité : **Pack** — module OCA `product_pack`, case *« Est un pack ? »* = booléen `pack_ok`, onglet *Pack*)* — coffrets, assortiments, multi-produits pensés pour une occasion ou un usage (kit colombo, kit apéritif, kit découverte…) ;
4. **Catégories** — famille de produits ;
5. **Origines** — repère géographique.
...
#### Présentation front (implémentation)

Alignée avec les principes §2 (rail **secondaire**, **pas** de message porté par un autoplay) :

- **Disposition** : **cinq cartes** dans un **rail horizontal** défilable (`overflow-x` sur le viewport) ; **scroll-snap** sur les largeurs où les cartes dépassent une « page » visuelle, désactivé sur les grands écrans pour laisser le pas à pas JS voir ~quatre cartes entières selon la largeur du conteneur (**container queries** `cqi`).
- **Contrôle visiteur** : **pas de défilement automatique** ; navigation **manuelle** par **boutons** « précédent » et « suivant », par **glissement / trackpad / barre de défilement** native du rail, ou par **flèches** gauche / droite lorsque le rail a le **focus** clavier.
- **Boucle** : en bout de rail, le bouton « suivant » ramène au **début** (saut **instantané**) ; au **début**, « précédent » ramène en **fin** de rail (même principe), pour garder une grille courte sans impasse.
- **Accessibilité** : `aria-controls` sur les boutons vers l’identifiant du rail ; région `role="region"` `aria-roledescription="carrousel"` reliée au titre de section ; rail `tabindex="0"` et `aria-label` explicite.
- **Mise en page** : en-tête de section (filet + titre + accroche) **centré** ; **rythme vertical** avec le **hero** et les blocs suivants harmonisé sur les tokens de section (`$ckr-section-py-mobile` / `$ckr-section-py-desktop`) — padding bas du hero aligné sur les sections, pas de réduction spéciale sur la première section après le hero.
- **Sens du scroll technique** : `dir="ltr"` sur le rail (scroll horizontal **stable** même si une locale ou le thème impose un RTL global).
- **Fichiers** : `views/snippets/ckr_entries.xml`, `static/src/scss/components/_entries.scss`, `static/src/js/ckr_entries_carousel.js` (bundle **`web.assets_frontend`** dans `__manifest__.py`).
...
- **Hero** + **Explorer** (cinq portes : Promotions, Collections, **Kits** *(front)* / **Pack** *(back-office)*, Catégories, Origines) + **mise en avant fournisseur / origine** + **sélection produits** + **bloc éditorial** (collection / Offrir / Recettes selon contenu disponible) + **bloc confiance** + **footer** personnalisé.
