...
1. **Standard Odoo d’abord** ([ADR-CKR-001](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-001)) : aucune logique métier promotionnelle parallèle côté CK. Le socle promo est celui activé sur l’instance (voir §2).
...
4. **Construction CK minimale et présentationnelle** ([ADR-CKR-002](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-002)) : la couche CK habille et fait le lien Explorer → `/shop` ; elle ne recompose pas la logique de remise.
...
- [ARCHITECTURE_DECISION_RECORD.md](../ARCHITECTURE_DECISION_RECORD.md) — ADR-CKR-001, 007, 008.
