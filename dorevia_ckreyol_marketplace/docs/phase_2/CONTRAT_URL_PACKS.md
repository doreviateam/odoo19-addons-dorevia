...
| **Prérequis actés** | Source de vérité **`product.template.pack_ok`** ([SPEC_SHOP_PORTES §4.3](SPEC_SHOP_PORTES.md)) ; convergence commerciale **`/shop`** ([ADR-CKR-007](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-007)) ; libellé visiteur **Kits** et règle de bi-lexique front / back-office ([ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008)). |
...
> **Convention lexicale interne au document** — conformément à la règle de bi-lexique [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) :
...
1. **Standard Odoo / brique OCA installée d’abord** ([ADR-CKR-001](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-001)) : pas de logique métier parallèle.
2. **Convergence commerciale unique sur `/shop`** ([ADR-CKR-007](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-007)) : pas d’univers commercial alternatif hors boutique native.
3. **Source de vérité unique** : `product_pack.pack_ok`. Pas de second marqueur « pack » côté CK.
4. **Construction CK minimale et présentationnelle** ([ADR-CKR-002](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-002)) : la couche CK habille, elle ne recompose pas la logique.
5. **Bi-lexique front / back-office** ([ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008)) : les **URL visibles** et les **libellés** côté visiteur utilisent la grille **Kits** (donc URL front `/kits`) ; les **paramètres et conventions internes** restent sur la grille **Pack** (domaine `pack_ok=True`, variable interne `ckr_mode=pack`…). Une option qui **force** l’exposition de « pack » côté visiteur (URL, titre) doit être écartée ; une option qui **force** l’exposition de « kits » côté code (nom de champ, filtre, server action) doit l’être aussi.
...
- Route non dédiée : page `website.page` pointant sur le template `ckr_page_compositions`, URL **`/kits`** (alignée sur le libellé visiteur [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008)), contenu éditorial « Bientôt disponible ».
...
Créer une **`product.public.category`** dédiée libellée **« Kits »** côté visiteur (racine ou sous-arbre) et y **rattacher** tous les produits avec `pack_ok=True`. URL obtenue : **`/shop/category/<id>-kits`** (forme standard Odoo, slug aligné sur le libellé visiteur [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008)).
...
- **Sémantique impure** (C1) : les autres catégories publiques structurent par **famille de produit** (biscuits, boissons, épicerie…) ; glisser « Kits » au même niveau **mélange** *type d’article* et *statut d’assemblage*. Tension directe avec [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) qui distingue explicitement **Catégories** (famille) et **Kits** (assemblage, logique Pack en interne).
...
URL type : **`/shop?ckr_mode=pack`** (le paramètre est une **convention interne** alignée sur la **grille back-office** de la règle de bi-lexique [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) ; la valeur alternative `ckr_mode=kits` serait cohérente avec la grille front, voir §11 *Décision attendue*).
...
**Très aligné doctrinalement** (C1, C2, C3). Le coût est essentiellement cosmétique (URL, breadcrumb) et se traite par la couche présentationnelle légitime ([ADR-CKR-002](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-002)). Fort candidat.
...
Créer une route CK **`/kits`** (contrôleur, alignée libellé visiteur [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008)) qui, au choix :
...
**Avantages** : 
- URL visible **lisible et alignée sur le libellé visiteur** (`/kits`) — la règle de bi-lexique [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) est **effective**, pas seulement documentée.
...
1. **C1 — règle de bi-lexique effective** ([ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008)) : l’URL visible est **`/kits`** (grille front), les conventions internes restent **Pack** (`ckr_mode=pack`, `pack_ok`). La règle est **matérialisée**, pas seulement documentée.
...
|---------|--------------|--------------------|-------------------|
| **URL visiteur** (carte Explorer, partage, copy marketing) | `/kits` | Front (Kits) | [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) |
| **URL technique canonique** (`<link rel="canonical">`) | `/shop?ckr_mode=pack` | Back-office (Pack) | [ADR-CKR-007](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-007) (forme native `/shop`) + [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) |
| **Paramètre CK** | `ckr_mode=pack` | Back-office (Pack) | [ADR-CKR-001](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-001) (source de vérité native) + [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) |
...
| **Filtre domaine produit** | `("pack_ok", "=", True)` | Back-office | [ADR-CKR-001](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-001) + `product_pack` |
| **Mécanisme `/kits`** | Redirection HTTP **301** vers `/shop?ckr_mode=pack` | Transition front → technique | [ADR-CKR-007](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-007) (stubs = transitoires) |
| **Sort du stub `website.page` `/kits`** | **Retiré** (ou dépublié) — la route `/kits` devient une **redirection** portée par le contrôleur CK | Nettoyage | [ADR-CKR-007](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-007) |
| **Titre / surtitre rendu sur `/shop?ckr_mode=pack`** | Libellé visiteur « **Kits** » (titre de porte, breadcrumb CK) | Front (Kits) | [ADR-CKR-002](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-002) (habillage front) + [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) |
| **Combinatoire native** (`search`, `attrib`, `order`, `page`) | **Préservée** — le paramètre CK vient en addition, ne remplace rien | Compatibilité standard | [ADR-CKR-001](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-001) |
| **Référence invalide / catalogue vide** | État vide **natif** `website_sale` + éventuelle note CK | Robustesse | [ADR-CKR-001](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-001) |
...
- L’URL **canonique** vue par les moteurs est **`/shop?ckr_mode=pack`** — ce qui renforce [ADR-CKR-007](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-007) (pas de vitrine parallèle à `/shop`).
...
- [ADR-CKR-001](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-001) — standard Odoo d’abord.
- [ADR-CKR-002](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-002) — spécifique front-end uniquement.
- [ADR-CKR-007](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-007) — convergence `/shop`.
- [ADR-CKR-008](../ARCHITECTURE_DECISION_RECORD.md#adr-ckr-008) — cinq portes d’exploration, libellés pluriel, règle de bi-lexique front / back-office.
